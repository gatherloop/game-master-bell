...
Carried over from the API's standalone-repo history (PRD-v2 phase A5, the
final step of that repo's API track) — the content below (Firebase
decommission, `/healthz` monitoring, passcode rotation) is unchanged by the
[PRD-v3](../../../docs/PRD-v3.md) move into this monorepo; only the file's
location and cross-links moved. By this point the old Firebase Cloud
Function and native Android receiver had already been removed from the bell
app (superseded by v3's own native receiver, see [PRD-v3](../../../docs/PRD-v3.md)),
and the production call path has been running on this API since PRD-v2
phase B2. Nothing references the v1 Firebase project, so it stayed safe to
tear down.

This document covers the three things phase A5 was scoped to: closing out
